defmodule TicketAgent.Generators.OrderPdfGeneratorTest do
  ...
  describe "generate_order_pdf_binary" do
    test "works for orders that are processing and tickets that are processing", %{order: order, ticket: ticket} do
      ...
      assert response =~ "https://chart.googleapis.com/"
  ...
  describe "generate_order_pdf_file" do
    test "it names the file", %{order: order} do
      ...
      assert order.slug == OrderPdfGenerator.generate_order_pdf_file(order)
    end
  end
end
