defmodule TicketAgent.State.TicketStateTest do
  ...
  describe "lock_processing_tickets" do
    ...
    test "affects processing", %{timestamp: timestamp} do
      ...
      assert inspect(query) ==
               ~s(#Ecto.Query<from t in TicketAgent.Ticket, where: t.order_id == ^\"#{order.id}\", where: t.status == \"processing\">)
  ...
  describe "unlock_tickets_to_processing_for_order" do
    ...
    test "affects locked tickets" do
      ...
      assert inspect(query) ==
               ~s(#Ecto.Query<from t in TicketAgent.Ticket, where: t.order_id == ^\"#{order.id}\", where: t.status == \"locked\">)
  ...
  describe "purchase_processing_tickets_for_order" do
    ...
    test "affects locked", %{timestamp: timestamp} do
      ...
      assert inspect(query) ==
               ~s(#Ecto.Query<from t in TicketAgent.Ticket, where: t.order_id == ^\"#{order.id}\", where: t.status in [\"locked\", \"processing\"]>)
  ...
  describe "email_purchased_tickets_for_order" do
    test "does not affect available" do
      order = insert(:order)
      ticket = insert(:ticket, status: "available", order: order)
      multi = TicketState.email_purchased_tickets_for_order(order)
    ...
    test "affects purchased", %{timestamp: timestamp} do
      ...
      multi = TicketState.email_purchased_tickets_for_order(order, timestamp)
      ...
      assert inspect(query) ==
               ~s(#Ecto.Query<from t in TicketAgent.Ticket, where: t.order_id == ^\"#{order.id}\", where: t.status == \"purchased\">)
  ...
  describe "set_ticket_checkedin" do
    ...
    test "affects purchased", %{timestamp: timestamp, user: user} do
      ...
      assert inspect(query) ==
               ~s(#Ecto.Query<from t in TicketAgent.Ticket, where: t.id == ^\"#{ticket.id}\", where: t.status in [\"emailed\", \"purchased\"]>)

      assert updates == [
               set: [status: "checkedin", checked_in_at: timestamp, checked_in_by: user.id]
             ]
  ...
  describe "lock_tickets" do
    test "it makes sure they are all updated", %{timestamp: timestamp} do
      ...
      assert inspect(query) ==
               "#Ecto.Query<from t0 in TicketAgent.Ticket, join: t1 in subquery(from t in TicketAgent.Ticket,\n  where: t.listing_id == ^\"#{
                 listing.id
  ...
  describe "filter_created_tickets" do
    ...
    test "gets 10 to create" do
      ...
      tickets =
        Enum.reduce(1..10, tickets, fn counter, acc ->
          acc ++
            [
              %{
                "listing_id" => listing.id,
  ...
  describe "load_minimum_locked_until" do
    test "gets the minimum" do
      ...
      {updated_order, tickets, locked_until} =
        TicketState.load_minimum_locked_until(order, listing.id)
      ...
      refute DateTime.to_iso8601(locked_until) ==
               NaiveDateTime.to_iso8601(old_ticket.locked_until) <> "Z"
