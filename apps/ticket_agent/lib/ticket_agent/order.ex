defmodule TicketAgent.Order do
  ...
  schema "orders" do
    ...
    belongs_to(:refunder, User, references: :id, foreign_key: :refunded_by, type: Ecto.UUID)

    belongs_to(:credit_card, CreditCard,
      references: :id,
  ...
  def additional_ticket_emails(%Order{} = order) do
    ...
    order =
      case Ecto.assoc_loaded?(order.user) do
        true -> order
    ...
    |> Enum.filter(fn ticket ->
      ticket.status == "purchased" && ticket.guest_email != order.user.email
    end)
    |> Enum.map(fn ticket ->
