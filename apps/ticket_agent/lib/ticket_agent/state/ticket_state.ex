defmodule TicketAgent.State.TicketState do
  ...
  def email_purchased_tickets_for_order(order, timestamp \\ NaiveDateTime.utc_now()) do
    Logger.info("email_purchased_tickets_for_order->order_id       = #{order.id}")
    Logger.info("email_purchased_tickets_for_order->timestamp      = #{inspect(timestamp)}")
  ...
  def filter_created_tickets(order, listing_id, input_tickets, group) do
    ...
    Logger.info(
      "find_existing_tickets->existing_tickets count: #{inspect(Enum.count(existing_tickets))}"
    )
      ...
      Enum.find(existing_tickets, fn existing_ticket ->
        existing_ticket.id == ticket["ticket_id"] ||
          (existing_ticket.guest_name == ticket["name"] &&
             ...
             existing_ticket.group == ticket["group"] && existing_ticket.price == ticket["price"])
      end)
      |> is_nil
    end)
  end
