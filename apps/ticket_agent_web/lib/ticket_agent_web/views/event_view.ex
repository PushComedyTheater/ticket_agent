defmodule TicketAgentWeb.EventView do
  ...
  def purchased_order_list(conn, orders) do
    Enum.map_join(orders, ", ", fn order ->
      safe_to_string(
        link(event_date(order.completed_at), to: order_path(conn, :show, order), target: "_blank")
      )
    end)
  end
  ...
  def full_event_date(%{listing_count: listing_count, start_at: start_at})
      when listing_count == 1 do
    start_at
    |> Calendar.DateTime.shift_zone!("America/New_York")
