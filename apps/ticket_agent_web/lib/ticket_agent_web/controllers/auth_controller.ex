defmodule TicketAgentWeb.AuthController do
  ...
  def callback(conn, %{"provider" => provider, "code" => code}) do
    ...
    with {:ok, client} <- get_token!(provider, code),
         {:ok, %{name: name, email: email, extra_details: extra_details}} <-
           get_user(provider, client) do
      user =
        User.find_or_create_with_credentials(
          name,
          email,
  ...
  defp authorize_url!("facebook", _params),
    do:
      TicketAgent.Facebook.authorize_url!(
        scope: "email,public_profile,user_birthday,user_friends,user_location,user_photos"
      )

  defp authorize_url!("google", _params),
    do:
      TicketAgent.Google.authorize_url!(
        scope: "https://www.googleapis.com/auth/plus.profile.emails.read"
      )

  defp authorize_url!("linkedin", _params),
    do: TicketAgent.LinkedIn.authorize_url!(scope: "r_basicprofile r_emailaddress")

  defp authorize_url!("microsoft", _params),
    do:
      TicketAgent.Microsoft.authorize_url!(
        scope: "wl.emails,wl.basic,wl.postal_addresses,wl.phone_numbers"
      )
  ...
  defp get_user("amazon", client) do
    with {:ok, %{body: %{"email" => email, "name" => name} = result}} <-
           OAuth2.Client.get(client, "https://api.amazon.com/user/profile?access_token") do
  ...
  defp get_user("facebook", client) do
    case OAuth2.Client.get(
           client,
           "/me?fields=about,id,birthday,devices,email,first_name,gender,is_verified,last_name,location,name,timezone,verified,website"
         ) do
      {:ok, %{body: user}} ->
        {:ok,
         %{
  ...
  defp get_user("linkedin", client) do
    ...
    with {:ok,
          %{
            body:
              %{"formattedName" => name, "pictureUrl" => avatar, "emailAddress" => email} = body
          }} <- OAuth2.Client.get(client, user_query, [], skip_url_encode_option()) do
  ...
  defp get_user("google", client) do
    with {:ok, %{body: %{"id" => id} = result}} <-
         ...
         {:ok,
          ...
          }} = OAuth2.Client.get(client, "https://www.googleapis.com/plus/v1/people/#{id}") do
  ...
  defp get_user("microsoft", client) do
    with {:ok, %{body: %{"name" => name, "emails" => %{"account" => email}} = body}} <-
           OAuth2.Client.get(client, "https://apis.live.net/v5.0/me"),
         {:ok, %{headers: headers, status_code: 302}} <-
           OAuth2.Client.get(client, "https://apis.live.net/v5.0/me/picture"),
  ...
  defp get_user("universe", client) do
    with {:ok,
          ...
          }} <- OAuth2.Client.get(client, "https://www.universe.com/api/v2/current_user") do
      {:ok,
       %{name: "#{first_name} #{last_name}", avatar: avatar, email: email, extra_details: body}}
    else
      {:error, response} ->
        Logger.error("Error: #{inspect(response)}")
        {:user_error, "Unknown error, contact support"}
