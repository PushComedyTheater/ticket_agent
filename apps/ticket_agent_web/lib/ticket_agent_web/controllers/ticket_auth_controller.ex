  ...
  def show(conn, %{"listing_id" => listing_id, "token" => token} = params) do
    case UserFinder.find_guest_by_token(token) do
      nil ->
        ...
        |> put_flash(
          :error,
          "There was an error with your submission.  Your link has expired.  Please try again or contact support@pushcomedytheater.com."
        )
  ...
  def create(conn, %{"registration" => registration_params, "listing_id" => listing_id} = params) do
    case UserFinder.find_guest_by_email(registration_params["email"]) do
      ...
      user ->
        ...
        |> put_flash(
          :error,
          "Please check your email for information on how to continue your guest checkout."
        )
        |> redirect(to: "/ticket_auth?listing_id=#{listing_id}")
        |> halt()
    end
  ...
  defp register_user(conn, %{"registration" => registration_params} = params) do
    ...
    |> case do
      ...
      {:error, _} ->
        ...
        |> put_flash(
          :error,
          "There was an error with your submission.  Please try again or contact support@pushcomedytheater.com."
        )
