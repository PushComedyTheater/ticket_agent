defmodule TicketAgentWeb.Admin.EmailTemplateController do
  ...
  def show(conn, %{"type" => "order_pdf", "id" => order_slug}) do
    ...
    email = TicketAgent.Emails.OrderEmail.order_receipt_email(order.id)

    output = "<pre>" <> inspect(email) <> "</pre>"
    conn
  ...
  def show(conn, %{"type" => "ticket_pdf", "id" => ticket_slug}) do
    ...
    Logger.info "Loading order id #{inspect order.slug}"
    ...
    html = EEx.eval_file(template, [url: url, tickets: [ticket], listing: listing, ticket_count: ticket_count, order: order, host: @host])
